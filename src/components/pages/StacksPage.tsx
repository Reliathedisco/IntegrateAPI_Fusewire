...
export default function StacksPage() {
  ...
  const preset = selectedSlug ? stackPresets.find((p) => p.slug === selectedSlug) : null
  const installedIntegrations = preset
    ? (preset.integrationIds
        .map((id) => getIntegrationBySlug(id))
        .filter((i): i is NonNullable<ReturnType<typeof getIntegrationBySlug>> => i != null))
    : []
  ...
  return (
    <div className="container">
      ...
      <p className="pageLead">
        Choose a preset to get a CLI command and the list of integrations that will be installed.
      </p>
      ...
      <div className="presetGrid">
        {stackPresets.map((p) => (
          <button
            key={p.id}
            type="button"
            className={`presetCard ${selectedSlug === p.slug ? "presetCardActive" : ""}`}
